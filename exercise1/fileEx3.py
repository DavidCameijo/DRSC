...
def main(stop_time, it,folder_results):
    ...
    size = 5
    json_topology = json.load(open('data/networkDefinition.json'))
    t.load(json_topology)
    num_nodes = t.G.number_of_nodes()
    num_edges = t.G.number_of_edges()
    print(f"Number of nodes: {num_nodes}, Number of edges: {num_edges}")
    print("Nodes with attributes:", t.G.nodes(data=True))
    print("Edges with attributes:", t.G.edges(data=True))

    #t.G = nx.generators.barabasi_albert_graph(size, 2) # In NX-lib there are a lot of Graphs generators
    
    # Definition of mandatory attributes of a Topology
    ## Attr. on edges
    # PR and BW are 1 unit
    #attPR_BW = {x: 0 for x in t.G.edges()}
    ...
    nx.write_gexf(t.G,folder_results+"graph_from_json.gexf") # you can export the Graph in multiples format to view in tools like Gephi, and so on.
