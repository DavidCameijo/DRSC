"""
    ...
    Barabasi-Albert topology (50 nodes, m=2), 5 applications,
...
def main(stop_time, it, folder_results):
    ...
    for x in t.G.nodes():
        if 0 <= x <= 4:
            ...
            # Fog nodes (39-48 are designated sink nodes)
            attRAM[x] = 8192
            attIPT[x] = 1000
        elif x == 49:
    ...
    """
    Deploy users (one source per GW node 0-4, one per app)
    """
...
if __name__ == '__main__':
    ...
    # Per-app breakdown example (App2, id=2)
    ...
    plt.figure()
    avg_latency.plot(kind='bar', title='Average Latency per Application', ylabel='Latency (ms)', xlabel='App ID')
    plt.tight_layout()
    ...
    node_usage.plot(kind='bar', title='Node Usage (Task Allocations)', ylabel='Tasks Processed', xlabel='Node ID')
    plt.tight_layout()
    ...
    if 'size' in dfl.columns:
        ...
        bw_app.plot(kind='bar', title='Bandwidth Consumption per App', ylabel='Total Bytes', xlabel='App ID')
    else:
        dfl.groupby('app').size().plot(kind='bar', title='Bandwidth (Message Counts) per App', ylabel='Total Messages', xlabel='App ID')
    plt.tight_layout()
    ...
    if 'inst' in df.columns:
        ...
        cpu_app.plot(kind='bar', title='CPU (Instructions) Consumption per App', ylabel='Total Instructions', xlabel='App ID')
    else:
        df.groupby('app').size().plot(kind='bar', title='CPU proxy (Requests) per App', ylabel='Total Requests', xlabel='App ID')
    plt.tight_layout()
